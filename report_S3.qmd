---
title: "Supplementary Report S3. Report with ASV post-clustering into OTU. Terroir and farming practices drive arbuscular mycorrhizal fungal communities in French vineyard."
format:
  html:   
    page-layout: full
    theme: 
...
```{r}
d_vs_blast <- tar_read(d_vs_blast)
d_vs <- tar_read(d_vs)

sam_data_updated <- read.delim("data/samples_data/sam_data_english.csv")
rownames(sam_data_updated) <- sam_data_updated[,1]
d_vs_blast <- update_sam_data(d_vs_blast, sam_data_updated, overwrite = TRUE)
d_vs <- update_sam_data(d_vs, sam_data_updated, overwrite = TRUE)
```
...
d_vs <- clean_pq(subset_samples(d_vs, compartment != "Mycelium"))
```
...
hist(log10(sample_sums(d_vs)), breaks = 40)
summary(sample_sums(d_vs))
sd(sample_sums(d_vs))
```
...
# VALUE
seq_mucoromycota <-
  sum(taxa_sums(d_vs@otu_table)[as.vector(d_vs@tax_table[, "Class_PR2"] ==
...
seq_chelicerata <-
  sum(taxa_sums(d_vs@otu_table)[as.vector(d_vs@tax_table[, "Order_PR2"] ==
    "Chelicerata")], na.rm = T)
asv_chelicerata <-
  sum(taxa_sums(d_vs@otu_table)[as.vector(d_vs@tax_table[, "Order_PR2"] ==
...
df <- data.frame(
  "Proportion of sequences" = c(
    round(
      sum(d_vs_blast@otu_table) / sum(d_vs@otu_table), 3
    ),
    round(
      sum(d_muco@otu_table) / sum(d_vs@otu_table), 3
    )
  ),
  "Proportion of OTU" = c(
    round(
      ntaxa(d_vs_blast) / ntaxa(d_vs), 3
    ),
    round(
      ntaxa(d_muco) / ntaxa(d_vs), 3
    )
  )
)
...
sequences per samples is 6768.

```{r}
spores_roots_samples <- sample_names(d_vs_blast)[d_vs_blast@sam_data$compartment!="Mycelium"]
...
tw <- track_wkflow(
  list(
    ...
    "OTU after vsearch reclustering at 97%" =clean_pq(subset_samples(tar_read(d_vs), compartment != "Mycelium")),
    "Maarjam Filtered" =   clean_pq(subset_samples(tar_read(d_vs_blast), compartment != "Mycelium")),
    "PR2 Filtered" = d_muco,
    "Roots only" = d_roots,
